...
static void 
startTracingCwnd (uint32_t nNodes){
    for(uint32_t i = 0 ; i < nNodes; i++){
...
main(int argc, char *argv[])
{
    ...
    uint32_t bytesToSend = 100 * 1e6; // 40 MB
    ...
    for (uint32_t i = 0; i < nNodes; i++)
    {
        ...
        //tmp_source.SetAttribute ("MaxBytes", UintegerValue (bytesToSend));
 ...
 // Configuring file stream to write the Qsize
    ...
    bottleneckTransimittedStream = ascii_qsize_tx.CreateFileStream(dir+bottleneckTxFileName+".txt");
    AsciiTraceHelper ascii_dropped;
    dropped_stream = ascii_dropped.CreateFileStream (dir+droppedTrFileName + ".txt");
    ...
    Simulator::Schedule( Seconds(stime+startTracing), &writeCwndToFile, nNodes);
