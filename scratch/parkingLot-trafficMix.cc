...
std::string dir = "result-parkingLot-TM/";
uint32_t prev = 0;
...
// tcp = 70% -> 42 bulksend
// udp = 20% -> 12 udp
// short lived flows = 10% -> 6 flows
uint32_t n_tcp_flows = std::round(nNodes * 0.7);
uint32_t n_udp_flows = std::round(nNodes * 0.2);
uint32_t n_short_tcp_flows = nNodes - (n_tcp_flows + n_udp_flows);

uint32_t total_udp_flows = 3* n_udp_flows;
uint32_t total_short_tcp_flows = 3* n_short_tcp_flows;
...
std::vector<uint32_t> numOfObs(numOfQueue, 0);
...
void TraceQueueSizeTc(int queueIdx) {
    ...
    *(tc_qSize_streamV[queueIdx])->GetStream()
    ...
    if ((numOfObs[queueIdx] != 0) && (numOfObs[queueIdx] % Q_WINDOW == 0)) {
    ...
    qSizeData[queueIdx][numOfObs[queueIdx] % Q_WINDOW] = queueDiscV[queueIdx]->GetCurrentSize().GetValue() + 1;
    numOfObs[queueIdx]++;
}
...
std::vector<double> sumWin(numOfQueue, 0);
...
static void CwndTracer(uint32_t node, uint32_t oldval, uint32_t newval) {
    ...
    sumWin[Q_SECOND] += (oldVal - prevWin[node]); 
    //sumWin += (oldVal - prevWin[node]); 
    prevWin[node] = oldVal;

    static int currT = -1, prevT = -1;
    if(currT == -1) currT = Simulator::Now().GetSeconds();
    prevT = Simulator::Now().GetSeconds();
    if(prevT- currT> 10)
    {
        currT = Simulator::Now().GetSeconds();
        std::cout<<"Time Spent sec: " << currT<<"s"<<std::endl;
    }
#if 0
    if (newval < oldval) {
        // loss_events[node] = 1;
...
static void start_tracing_timeCwnd(uint32_t nNodes) {
    for (uint32_t i = 0; i < nNodes; i++) {
        ...
        std::string fileName = dir + "node-" + std::to_string(i + 3) + ".cwnd";
...
{
// FIXME: only for long lived flows
    int totCount = 0;
...
int main(int argc, char *argv[])
{
    ...
    std::string zc_trace_filename = "zeroCrossingTrace-parkingLot";
    std::string dropped_trace_filename = "droppedPacketTrace-parkingLot";
    std::string bottleneck_tx_filename = "bottleneckTx-parkingLot";
    std::string tc_qsize_trace_filename = "tc-qsizeTrace-parkingLot";
    std::string rttFileName = "RTTs";

    std::string parametersFileName = "parameters";
    float stop_time = 700;
    float start_time = 0;
    float start_tracing_time = 15;
    ...
    NS_LOG_UNCOND("tcp: "<<total_tcp_flows<<", udp: "<<total_udp_flows<<", short lived: "<<total_short_tcp_flows);
    ...
    uint16_t port = 50000;
    Address sinkAddress[totalSourceNodes];
    ApplicationContainer sinkApp[totalSourceNodes];

    Ptr<Socket> ns3TcpSocket[totalSourceNodes];
    ApplicationContainer sourceApps[totalSourceNodes];
    ...
    double maxTime = start_time;
    double stime = start_time;
    ////// tcp
    // sink to all nodes
    PacketSinkHelper packetSinkHelper_tcp( "ns3::TcpSocketFactory", InetSocketAddress(Ipv4Address::GetAny(), port));

    // Configuring the application at each source node.
    for (uint32_t i = 0; i < n_tcp_flows; i++) {
        sinkAddressD1[i] = *(new Address(InetSocketAddress(D1Ip_v[i].GetAddress(0), port)));
        sinkAppD1[i] = packetSinkHelper_tcp.Install(D1[i].Get(0));
        sinkAppD1[i].Start(Seconds(start_time));
        sinkAppD1[i].Stop(Seconds(stop_time));
        ...
        sinkAddressD2[i] = *(new Address(InetSocketAddress(D2Ip_v[i].GetAddress(0), port)));
        sinkAppD2[i] = packetSinkHelper_tcp.Install(D2[i].Get(0));
        sinkAppD2[i].Start(Seconds(start_time));
        sinkAppD2[i].Stop(Seconds(stop_time));
        BulkSendHelper tmp_source2( "ns3::TcpSocketFactory", InetSocketAddress(D2Ip_v[i].GetAddress(0), port));
        // Set the amount of data to send in bytes.  Zero is unlimited.
        ...
        gap = expRandomVariable->GetValue();
        stime += gap;


        sinkAddressD3[i] = *(new Address(InetSocketAddress(D3Ip_v[i].GetAddress(0), port)));
        sinkAppD3[i] = packetSinkHelper_tcp.Install(D3[i].Get(0));
        sinkAppD3[i].Start(Seconds(start_time));
        sinkAppD3[i].Stop(Seconds(stop_time));
        BulkSendHelper tmp_source3( "ns3::TcpSocketFactory", InetSocketAddress(D3Ip_v[i].GetAddress(0), port));
        // Set the amount of data to send in bytes.  Zero is unlimited.
    ...
    ///// udp
    // Attach sink to all nodes
    PacketSinkHelper packetSinkHelper_udp( "ns3::UdpSocketFactory", InetSocketAddress(Ipv4Address::GetAny(), port));
    ...
    for (uint32_t i = n_tcp_flows; i < n_tcp_flows+n_udp_flows; i++) {
        // packet sink
        sinkAddressD1[i] = *(new Address(InetSocketAddress(D1Ip_v[i].GetAddress(0), port)));
        sinkAppD1[i] = packetSinkHelper_udp.Install(D1[i].Get(0));
        sinkAppD1[i].Start(Seconds(stime));
        sinkAppD1[i].Stop(Seconds(stop_time));

        // source installtion
        OnOffHelper tmp_source1( "ns3::UdpSocketFactory", InetSocketAddress(D1Ip_v[i].GetAddress(0), port));
        // Continuous transmission always on
        tmp_source1.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=1]"));
        tmp_source1.SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant=0]"));
        tmp_source1.SetAttribute("DataRate", StringValue("2Mbps"));
        tmp_source1.SetAttribute("PacketSize", UintegerValue(segmentSize));
        sourceAppsS1[i] = tmp_source1.Install(S1[i].Get(0));
        sourceAppsS1[i].Start(Seconds(stime));
        sourceAppsS1[i].Stop(Seconds(stop_time));
        ...
        // packet sink
        sinkAddressD2[i] = *(new Address(InetSocketAddress(D2Ip_v[i].GetAddress(0), port)));
        sinkAppD2[i] = packetSinkHelper_udp.Install(D2[i].Get(0));
        sinkAppD2[i].Start(Seconds(stime));
        sinkAppD2[i].Stop(Seconds(stop_time));

        // source installtion
        OnOffHelper tmp_source2( "ns3::UdpSocketFactory", InetSocketAddress(D2Ip_v[i].GetAddress(0), port));
        // Continuous transmission always on
        tmp_source2.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=1]"));
        tmp_source2.SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant=0]"));
        tmp_source2.SetAttribute("DataRate", StringValue("2Mbps"));
        tmp_source2.SetAttribute("PacketSize", UintegerValue(segmentSize));
        sourceAppsS2[i] = tmp_source2.Install(S2[i].Get(0));
        sourceAppsS2[i].Start(Seconds(stime));
        sourceAppsS2[i].Stop(Seconds(stop_time));
        ...
        // packet sink
        sinkAddressD3[i] = *(new Address(InetSocketAddress(D3Ip_v[i].GetAddress(0), port)));
        sinkAppD3[i] = packetSinkHelper_udp.Install(D3[i].Get(0));
        sinkAppD3[i].Start(Seconds(stime));
        sinkAppD3[i].Stop(Seconds(stop_time));

        // source installtion
        OnOffHelper tmp_source3( "ns3::UdpSocketFactory", InetSocketAddress(D3Ip_v[i].GetAddress(0), port));
        // Continuous transmission always on
        tmp_source3.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=1]"));
        tmp_source3.SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant=0]"));
        tmp_source3.SetAttribute("DataRate", StringValue("2Mbps"));
        tmp_source3.SetAttribute("PacketSize", UintegerValue(segmentSize));
        sourceAppsS3[i] = tmp_source3.Install(S3[i].Get(0));
        sourceAppsS3[i].Start(Seconds(stime));
        sourceAppsS3[i].Stop(Seconds(stop_time));
    ...
    // Attack sink to all nodes
    PacketSinkHelper packetSinkHelper( "ns3::TcpSocketFactory", InetSocketAddress(Ipv4Address::GetAny(), port));
    // install packet sink: it will be always on
    ...
    for (uint32_t i = (n_tcp_flows + n_udp_flows); i < nNodes; i++) {
        ...
        while (burst_end_time < stop_time) {
            BulkSendHelper tmp_source( "ns3::TcpSocketFactory", InetSocketAddress(D1Ip_v[i].GetAddress(0), port));

            tmp_source.SetAttribute("MaxBytes", UintegerValue(0));
            ApplicationContainer app = tmp_source.Install(S1[i].Get(0));
        ...
        burst_start_time = stime;
        burst_end_time = burst_start_time + 5;
        while (burst_end_time < stop_time) {
            BulkSendHelper tmp_source( "ns3::TcpSocketFactory", InetSocketAddress(D2Ip_v[i].GetAddress(0), port));

            tmp_source.SetAttribute("MaxBytes", UintegerValue(0));
            ApplicationContainer app = tmp_source.Install(S2[i].Get(0));
            app.Start(Seconds(burst_start_time));
            app.Stop(Seconds(burst_end_time));
        ...
        burst_end_time = burst_start_time + 5;
        while (burst_end_time < stop_time) {
            BulkSendHelper tmp_source( "ns3::TcpSocketFactory", InetSocketAddress(D3Ip_v[i].GetAddress(0), port));

            tmp_source.SetAttribute("MaxBytes", UintegerValue(0));
            ApplicationContainer app = tmp_source.Install(S3[i].Get(0));
            app.Start(Seconds(burst_start_time));
            app.Stop(Seconds(burst_end_time));
    ...
    *parameters->GetStream() << "Simulation Stop time: \t" << stop_time << std::endl;
    ...
    bottleneckTransimittedStreamV[Q_SECOND] = ascii_qsize_tx2.CreateFileStream(dir + bottleneck_tx_filename + "-2.txt");
    ...
    NS_LOG_UNCOND("Start tracing:  maxTime: " << maxTime);
    ...
    Simulator::Schedule(Seconds(maxTime), &updateCwndValues, nNodes);
    ...
    for (auto time = maxTime+ start_tracing_time; time < stop_time; time += 0.1) {
        ...
        Simulator::Schedule(Seconds(time), &TraceBottleneckTx, Q_FIRST); 
        Simulator::Schedule(Seconds(time), &TraceBottleneckTx, Q_SECOND); 
