...
static void TxPacket(Ptr<const Packet> p) { bottleneckTransimittedBytes += p->GetSize(); }
...
int main(int argc, char *argv[]) {
    ...
    float stop_time = 800; // simulation stop time 
    float start_time = 0;
    float start_tracing_time = 5; // start tracing sources
    bool enable_bot_trace = 0; // start tracing at router 0
    ...
    // write RTT for all client nodes
    ...
    for (uint32_t i = 0; i < n_tcp_flows; i++) {
        ...
        sinkApp[i] = packetSinkHelper_tcp.Install(nodes.Get(2 + nNodes + i));
    ...
    for (uint32_t i = n_tcp_flows; i < n_tcp_flows+n_udp_flows; i++) {
        ...
        // source installtion
        OnOffHelper tmp_source(
            "ns3::UdpSocketFactory",
            InetSocketAddress(rIp[i].GetAddress(0), port));
        // Continuous transmission
        // always on
        tmp_source.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=1]"));
        tmp_source.SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant=0]"));

        tmp_source.SetAttribute("DataRate", StringValue("2Mbps"));
        tmp_source.SetAttribute("PacketSize", UintegerValue(segmentSize));
