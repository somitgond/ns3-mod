...
static void writeCwndToFile(uint32_t n_nodes) {
    for (uint32_t i = 0; i < n_nodes; i++) {
        Config::ConnectWithoutContext(
            "/NodeList/" + std::to_string(i + 2) +
                "/$ns3::TcpL4Protocol/SocketList/0/CongestionWindow",
            MakeBoundCallback(&CwndTracer, cwnd_streams[i]));
    }
}

// initialize tracing cwnd streams
static void start_tracing_timeCwnd(uint32_t n_nodes) {
    for (uint32_t i = 0; i < n_nodes; i++) {
...
int main(int argc, char *argv[]) {
    ...
    cmd.AddValue("bytes_to_send", "Bytes to send using BulkSend",
                 bytes_to_send);
    ...
    cmd.AddValue("bottleneck_delay", "Delay of Bandwidth Link",
                 bottleneck_delay);
    cmd.AddValue("access_bandwidth", "Bandwidth of the branches",
                 access_bandwidth);
    cmd.AddValue("root_dir", "Root Directory of Project", root_dir);
    cmd.AddValue("qsize_trace_filename", "FileName to store qsize trace",
                 qsize_trace_filename);
    cmd.AddValue("dropped_trace_filename", "FileName to store dropped packets",
                 dropped_trace_filename);
    cmd.AddValue("bottleneck_tx_filename", "FileName to store bottlneck tra",
                 bottleneck_tx_filename);
    cmd.AddValue("stop_time", "Simulation stop time", stop_time);
    cmd.AddValue("start_time", "Simulation Start Time", start_time);
    cmd.AddValue("start_tracing_time", "Time to wait before tracing",
                 start_tracing_time);
    cmd.AddValue("enable_bot_trace", "Enable Tracing for whole simulation",
                 enable_bot_trace);
    cmd.Parse(argc, argv);
    ...
    // (QueueSize ("1p")));
    // Config::SetDefault (queue_disc + "::MaxSize", QueueSizeValue (QueueSize
    // (queue_size)));
    ...
    for (uint32_t i = 0; i < n_nodes; i++) {
        ...
        p2p_s[i].SetQueue(
            ...
            QueueSizeValue(QueueSize(std::to_string(0 / n_nodes) +
    ...
    *parameters->GetStream()
        << "sampling cwnd whenever it changes." << std::endl;
    ...
    *parameters->GetStream()
    ...
    Simulator::Schedule(Seconds(stime + start_tracing_time), &writeCwndToFile,
                        n_nodes);
